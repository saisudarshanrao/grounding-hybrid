...
reports, per method, the mean and spread of the per-fold span-level ROC-AUC, and the mean
...
from grounding_hybrid.signals import BASE_FEATS, S1_FEATS, S2_FEATS, load_signals  # noqa: E402
...
def main():
    ...
    ap.add_argument("--out", default=str(ROOT / "results" / "gating" / "dev_cv.json"))
    args = ap.parse_args()
    ...
    for canon in sorted(Path(args.canon_root).glob("*_K5")):
        ...
        print(f"\n# {run}  dev: {len(dev)} sentences / {dev['source_id'].nunique()} sources, "
    ...
    if summary:
        print("\n# Summary: mean dev-CV span AUC (rows = methods)")
