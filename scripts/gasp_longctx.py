...
the datasets in LONGCTX_DOMAINS; takes run_gasp.py's arguments plus --domain.
...
def load_domain_cases(domain, n_per_class, k_chunks, seed):
    ...
    sample = pos[:n] + neg[:n]
    print(f"      RAGBench {domain} (all splits) balanced: {n}/class from {len(pool)} pooled")
    cases = []
...
def main():
    ...
    domain = argv[i + 1]
    del argv[i:i + 2]
    run_gasp.load_ragbench_cases = lambda n, k, s: load_domain_cases(domain, n, k, s)
